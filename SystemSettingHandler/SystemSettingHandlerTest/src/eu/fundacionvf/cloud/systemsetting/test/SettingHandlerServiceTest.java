package eu.fundacionvf.cloud.systemsetting.test;
...
import eu.fundacionvf.cloud.systemsetting.SettingHandlerService;
...
public class SettingHandlerServiceTest extends
	...
	public void testDimScreen() {
		...
		try {
			...
			assertEquals(Build.VERSION.SDK_INT < 17 ? getService().MESSAGE_OK
					: "ERROR:  / changeDim /", getService().configure(info));
			info = new CloudIntent("SystemSettingHandler", 3610, 215);
			info.setParams("dim_screen", "1");
			assertEquals(Build.VERSION.SDK_INT < 17 ? getService().MESSAGE_OK
					: "ERROR:  / changeDim /", getService().configure(info));
			info = new CloudIntent("SystemSettingHandler", 3470, 215);
			info.setParams("dim_screen", "ssss");
			assertEquals("ERROR:  / changeDim /", getService().configure(info));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void testHapticFeedback() {
		try {
			...
			assertEquals("ERROR:  / changeHaptic /",
					getService().configure(info));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
